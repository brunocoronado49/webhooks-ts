...
export class GithubSha256Middleware {
  ...
  static async verifyGithubSignature(req: Request, res: Response, next: NextFunction) {
    const xHubsignature: string = `${req.header('x-hub-signature-256')}`;
    const body: string = JSON.stringify(req.body);
    const secret: string = envs.SECRET_TOKEN;
  ...
  private static async verifySignature(secret: string, header: string, payload: string) {
    try {
      ...
      let key = await crypto.subtle.importKey('raw', keyBytes, algorithm, extractable, [
        'sign',
        'verify',
      ]);
